...
9. **For slides with `fragments > 0` in `src/App.tsx`** (reveal-step slides — see `CLAUDE.md` → "Slide fragments"): capture each fragment state as a separate screenshot. Start at `http://localhost:5173/#/N/M` (fragment 0), screenshot, then for each k in 1..N press `ArrowRight` (via `evaluate_script` dispatching a `keydown`, or directly navigate to `#/col/row/k`), screenshot. Verify: (a) fragment 0 renders the intended initial state — often deliberately empty, don't flag empty-background as broken on a known fragment slide; (b) each advance produces a visible state change without console errors; (c) hash URL updates to include `/k` after each advance; (d) pressing `ArrowLeft` from the *next* slide returns to this slide's last fragment (hash `#/col/row/N`). Save as `/tmp/review/slide-N-fragment-K-desktop.png`.
