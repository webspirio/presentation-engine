      ...
      {STAGE_LABELS.map((label, i) => {
        ...
        return (
          <div key={label} className="flex flex-col items-center gap-2">
            ...
            />
            <span className="text-[10px] tracking-wide text-cyan-200/70" style={{ fontFamily: FONT_POPPINS }}>
              {label}
            </span>
          </div>
        )
    ...
    <section className="relative h-full w-full overflow-hidden">
      ...
      <div className="relative z-10 flex h-full w-full flex-col px-10 py-10 sm:px-16 sm:py-12">
        <div className="flex items-start justify-between gap-6">
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-3">
              <span aria-hidden className="text-3xl leading-none sm:text-4xl">{data.emoji}</span>
              <span
                className="text-cyan-300"
                style={{ fontFamily: FONT_DISPLAY, fontWeight: 600, fontSize: 'clamp(1rem, 1.4vw, 1.25rem)', letterSpacing: '0.02em' }}
              >
                {data.stageIndex}/5
              </span>
            </div>
            <span
              ...
              style={{ fontFamily: FONT_POPPINS, fontWeight: 500, fontSize: 'clamp(0.95rem, 1.2vw, 1.1rem)' }}
            >
              {data.stageLabel}
            </span>
          </div>
        ...
        <motion.h2
          ...
          initial="hidden"
          animate={stage(0)}
          className="mt-10 max-w-[18ch] text-balance text-[#F0F4F8] sm:mt-14"
          style={{
        ...
        >
          <span aria-hidden className="h-px w-10 shrink-0 bg-cyan-400/60" />
          <AlertTriangle aria-hidden className="h-4 w-4 shrink-0 text-cyan-300/90" />
          <p
            className="text-[#F0F4F8]/80"
            style={{ fontFamily: FONT_POPPINS, fontWeight: 400, fontSize: 'clamp(1.05rem, 1.5vw, 1.3rem)', lineHeight: 1.4 }}
          >
            {data.leak}
          </p>
        </motion.div>
        ...
        <div className="mt-auto flex flex-col gap-6 pt-8 md:flex-row md:items-end md:justify-between">
          ...
          >
            <motion.div
              ...
              style={{ fontFamily: FONT_POPPINS, fontWeight: 600, fontSize: '1.15rem' }}
            >
              {data.caseClient}
            </motion.div>
            {data.caseLocation && (
              <motion.div
                ...
                style={{ fontFamily: FONT_POPPINS, fontWeight: 400, fontSize: '0.9rem' }}
              >
                {data.caseLocation}
              </motion.div>
            )}
            <motion.p
              ...
              style={{ fontFamily: FONT_DISPLAY, fontWeight: 600, fontSize: 'clamp(1.05rem, 1.5vw, 1.3rem)', lineHeight: 1.35 }}
            >
              {data.caseResult}
            </motion.p>
